...
class InstagramPost(snscrape.base.Item):
    ...
    thumbnailUrl = snscrape.base._DeprecatedProperty('thumbnailUrl', lambda self: self.medium.thumbnailUrl,
                                                     'medium.thumbnailUrl')
    displayUrl = snscrape.base._DeprecatedProperty('displayUrl',
                                                   lambda self: None if self.isVideo else self.medium.fullUrl,
                                                   'medium.fullUrl')
...
class User(snscrape.base.Item):
    ...
    followersGranularity = snscrape.base._DeprecatedProperty('followersGranularity',
                                                             lambda self: self.followers.granularity,
                                                             'followers.granularity')
    followingGranularity = snscrape.base._DeprecatedProperty('followingGranularity',
                                                             lambda self: self.following.granularity,
                                                             'following.granularity')
    postsGranularity = snscrape.base._DeprecatedProperty('postsGranularity', lambda self: self.posts.granularity,
                                                         'posts.granularity')
    ...
    def _response_to_items(self, response):
        for node in response[self._edgeXToMedia]['edges']:
            ...
            if node['node']['is_video']:
                try:
                    variants = [
                        VideoVariant(url=node['node']['video_url'], width=node['node']['dimensions']['width'],
                                     height=node['node']['dimensions']['height'])
                    ]
                    medium = Video(thumbnailUrl=node['node']['thumbnail_src'], variants=variants,
                                   duration=int(node['node']['video_duration']) if 'video_duration' in node[
                                       'node'] else None, views=node['node']['video_view_count'])
...
class InstagramUserScraper(_InstagramCommonScraper):
    ...
    def _get_entity(self):
        ...
        ogDescriptionPattern = re.compile(
            '^(' + numPattern + ') Followers, (' + numPattern + ') Following, (' + numPattern + r') Posts - See Instagram photos and videos from (?:(.*?) \(@([a-z0-9_.]+)\)|@([a-z0-9_-]+))$')
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument('username', type=snscrape.utils.nonempty_string_arg('username'),
    ...
    def _response_to_items(self, response):
        for node in response['sections']:
            for media in node['layout_content']['medias']:
                ...
                medium = Photo(media['media']['image_versions2']['candidates'][-1]['url'],
                               media['media']['image_versions2']['candidates'][0]['url'])
                if 'video_versions' in media['media']:
                    variants = []
                    for version in media['media']['video_versions']:
                        variants.append(
                            VideoVariant(url=version['url'], width=version['width'], height=version['height']))

                    medium = Video(thumbnailUrl=media['media']['image_versions2']['candidates'][-1]['url'],
                                   variants=variants,
                                   duration=int(media['media']['video_duration']) if 'video_duration' in media[
                                       'media'] else None,
                                   views=media['media']['play_count'] if 'play_count' in media['media'] else None)
